/*jslint node:true, nomen:true, camelcase: false*/
/*global escape */
'use strict';
...
module.exports = {
	init: function (config) {
		...
		mailer = smtp(url,templates,logger);
	...
	createPasswordReset: function (req,res/*,next*/) {
      ...
      if (code !== 201) {
        logger.warn('[createPasswordReset]', code, message);
      }
	...
	createPasswordResetNext: function (req,res,next) {
      ...
      if (code !== 201) {
        logger.warn('[createPasswordResetNext]', code, message);
      }
			next();
		});
	},
	completePasswordReset: function (req,res/*,next*/) {
      ...
      if (code !== 200) {
        logger.warn('[completePasswordReset]', code, message);
      }
			res.send(code,message);
		});
	},
	completePasswordResetNext: function (req,res,next) {
		completePasswordReset(req,function (code,message) {
			...
			_.extend(req.activator,{code:code,message:message});
      if (code !== 200) {
        logger.warn('[completePasswordResetNext]', code, message);
      }
			next();
		});
	},
	createActivate: function (req,res/*,next*/) {
		createActivate(req,function (code,message) {
			res.send(code,message);
      if (code !== 200) {
        logger.warn('[createActivate]', code, message);
      }
		});
	},
	createActivateNext: function (req,res,next) {
		createActivate(req,function (code,message) {
			...
			_.extend(req.activator,{code:code,message:message});
      if (code !== 200) {
        logger.warn('[createActivateNext]', code, message);
      }
			next();
		});
	},
	completeActivate: function (req,res/*,next*/) {
      ...
      if (code !== 200) {
        logger.warn('[completeActivate]', code, message);
      }
			res.send(code,message);
		});
	},
	completeActivateNext: function (req,res,next) {
		completeActivate(req,function (code,message) {
			...
			_.extend(req.activator,{code:code,message:message});
      if (code !== 200) {
        logger.warn('[completeActivateNext]', code, message);
      }
			next();
		});
	}	
